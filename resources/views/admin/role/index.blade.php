...
@section('content')
                ...
                <div class="nav_cust_menu">
                    <ul>
                        ...
                        @can('view_role', $permission)
                            <li><a href="{{ route('roles') }}" class="active" title="Permission Panel">Permission Panel</a>
                            </li>
                        @endcan
                    </ul>
                </div>
                                    ...
                                    <div class="collaps_btns">
                                        @can('add_role', $permission)
                                            <a class="btn btn-success waves-effect waves-light roleModelForm"
                                                data-toggle="modal" title="Add User">Add New Role</a>
                                        @endcan
                                    </div>
                                </h2>
                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <span>
                                                @can('edit_role', $permission)
                                                    <a class="roleModelForm" data-toggle="modal" data-id="{{ $role->role_id }}"
                                                        data-name="{{ $role->name }}" data-slug="{{ $role->slug }}"
                                                        title="Edit Role" style="cursor: pointer;margin-right: 5px;"><i
                                                            class="fa fa-edit"></i>
                                                    </a>
                                                    @endcan @can('edit_role', $permission)
                                                    | <form id="delete-form-{{ $role->slug }}" method="post"
                                                        action="{{ route('role-delete', $role->slug) }}" style="display: none;">
                                                        @csrf
                                                ...
                                                "
                                                    ...
                                                    |
                                                    <button
                                                        class="btn btn-blue btn-sm text-primary show_hide{{ $role->role_id }}">Show
                                                        Permissions</button>
                                                @endcan
                                            </span>
                                        </div>
                                                                        ...
                                                                        @foreach ($row->sub_module as $value)
                                                                            <li class="list-inline-item mr-3">
                                                                                <?php $select = DB::table('permission_roles')
                                                                                    ...
                                                                                    ->get(); ?>
                                                                                <label
                                                                                    class="form-check-label">{{ ucfirst($value->name) }}</label>
                                                                                <div class="d-flex flex-wrap gap-2">

                                                                                    <input type="checkbox"
                                                                                        id="switch{{ $role->role_id }}{{ $row->module_id }}{{ $value->permission_id }}"
                                                                                        onclick="toggleStatus('switch{{ $role->role_id }}{{ $row->module_id }}{{ $value->permission_id }}', '{{ $value->permission_id }}', '{{ $role->role_id }}')"
                                                                                        switch="bool"
                                                                                        @foreach ($select as $selected) checked @endforeach
                                                                                        value="{{ $value->permission_id }}">
                                                                                    <label
                                                                                        for="switch{{ $role->role_id }}{{ $row->module_id }}{{ $value->permission_id }}"
                                                                                        data-on-label="Yes"
    ...
    <!-- Add role model -->
    <div class="modal fade" id="roleModelForm" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true"
