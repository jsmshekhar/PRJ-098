...
@section('content')
                ...
                <div class="nav_cust_menu">
                    <ul>
                        @can('view_complaint', $permission)
                            <li><a href="{{ route('complain-queries') }}" class="" title="User Panel">Complain & Queries</a>
                            </li>
                        @endcan
                        <li><a href="{{ route('complain-categories') }}" class="active" title="Permission Panel">Complain
                                Categories</a>
                        </li>
                    </ul>
                </div>
                ...
                <div class="card">
                    <div class="table-filter">
                        <form method="post" enctype="multipart/form-data"
                            action="{{ route('add-update-complain-category') }}" id="updateCategory" autocomplete="off">
                            @csrf
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="categoryModelForm" data-toggle="modal"
                                                        ...
                                                        data-role_id="{{ $category->role_id }}" title="Edit Category"
                                                        style="cursor: pointer;margin-right: 5px;"><i
                                                            class="fa fa-edit"></i>
                                                    </a> | <form id="delete-form-{{ $category->slug }}" method="post"
                                                        action="{{ route('caomplain-category-delete', $category->slug) }}"
                                                        style="display: none;">
                                                        @csrf
    ...
    <div class="modal fade" id="categoryModelForm" tabindex="-1" role="dialog" aria-labelledby="modalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="post" enctype="multipart/form-data" action="{{ route('add-update-complain-category') }}"
                    id="updateCategory" autocomplete="off">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <select class="form-control select2" name="role_id" id="role_id" required>
                                    <option value="">Select Role</option>
                                    @foreach ($roles as $key => $role)
                                        <option value="{{ $role->role_id }}">{{ $role->name }} ({{ $role->city }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
