...
@section('content')
                ...
                <div class="nav_cust_menu">
                    <ul>
                        @can('view_complaint', $permission)
                            <li><a href="{{ route('complain-queries') }}" class="active" title="User Panel">Complain & Queries</a>
                            </li>
                        @endcan
                        <li><a href="{{ route('complain-categories') }}" class="" title="Permission Panel">Complain
                                Categories</a>
                        </li>
                    </ul>
                </div>
                                            ...
                                            <div class="row">
                                                <div class="col-xl-3 col-md-6">
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input type="text" class="form-control" name="complain_id"
                                                            value="<?= isset($_GET['complain_id']) ? $_GET['complain_id'] : '' ?>" />
                                                    </div>
                                                </div>
                                                <div class="col-xl-3 col-md-6">
                                                    <div class="form-group mb-3">
                                                    ...
                                                    <select class="form-control selectBasic" name="category">
                                                        ...
                                                        @foreach ($categories as $key => $category)
                                                            <option value="{{ $category->slug }}"
                                                                <?= isset($_GET['category']) && $category->slug == $_GET['category'] ? 'selected' : '' ?>>
                                    ...
                                    <table id="tech-companies-1" class="table">
                                        ...
                                        <tbody>
                                            @foreach ($complains as $key => $complain)
                                                <tr>
                                                    <td>
                                                        <a class="complainModelForm" data-toggle="modal"
                                                            data-description="{{ $complain->description }}"
                                                            data-name="{{ $complain->name }}"
                                                            data-email="{{ $complain->email }}"
                                                            data-phone="{{ $complain->phone }}"
                                                            data-date="{{ dateFormat($complain->created_at) }}"
                                                            data-category="{{ $complain->complain_category }}"
                                                            data-cnumber="{{ $complain->complain_number }}"
                                                            data-slug="{{ $complain->slug }}"
                                                            data-cname="{{ $complain->category_name }}"
                                                            data-userid="{{ $complain->user_id }}"
                                                            data-role_id="{{ $complain->role_id }}"
                                                            title="View Description"
                                                            style="cursor: pointer;margin-right: 5px;">{{ $complain->complain_number }}
                                                        </a>
                                                    </td>
                                                        ...
                                                        @can('change_complaint_status', $permission)
                                                            <div class="d-flex flex-wrap gap-2">
                                                                <input type="checkbox" id="switch3{{ $key }}"
                                                                    ...
                                                                    switch="bool"
                                                                    {{ $complain->status_id == 1 ? 'checked' : '' }}
                                                                    value="{{ $complain->slug }}">
                                                                <label for="switch3{{ $key }}"
                                                                    data-on-label="Resolved" data-off-label="Pending"></label>
                                                            </div>
                                                        @endcan
                                                    </td>
                                                </tr>
    ...
    <div class="modal fade" id="complainModelForm" tabindex="-1" role="dialog" aria-labelledby="modalLabel"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    @can('change_assignment', $permission)
                        <div>
                            <form method="POST" action="{{ route('change-complain-assignment') }}"
                                id="changeAssignmentForm">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                        ...
                                        <div class="col-md-3">
                                            <div class="mb-3">
                                                <button type="submit" class="btn btn-success mb-3"
                                                    id="changeAssignment">Change</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
