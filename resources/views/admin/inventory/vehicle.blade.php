                                                ...
                                                <div class="col-xl-3 col-md-6">
                                                    <div class="form-group mb-3">
                                                        ...
                                                        <input type="text" class="form-control" name="cus_id"
                                                            value="<?= isset($_GET['cus_id']) ? $_GET['cus_id'] : '' ?>" />
                                                    </div>
                                                </div>
                                                <div class="col-xl-3 col-md-6">
                                                    <div class="form-group mb-3">
                                                ...
                                                <div class="col-xl-3 col-md-6 mb-3">
                                                    ...
                                                    <select class="form-control selectBasic" name="ev_cat">
                                                        ...
                                                        @foreach ($ev_category as $key => $ev_cat)
                                                            <option value="{{ $ev_cat }}"
                                                                <?= isset($_GET['ev_cat']) && $ev_cat == $_GET['ev_cat'] ? 'selected' : '' ?>>
                                                                {{ $ev_cat == 1 ? 'Two Wheeler' : 'Three Wheeler' }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                    ...
                                                    <select class="form-control selectBasic" name="hid">
                                                        ...
                                                        @foreach ($hubs as $key => $hub)
                                                            <option value="{{ $hub->hub_id }}"
                                                                <?= isset($_GET['hid']) && $hub->hub_id == $_GET['hid'] ? 'selected' : '' ?>>
                                                                {{ $hub->hubid }} ({{ $hub->city }})</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-xl-3 col-md-6 mb-3">
                                                    ...
                                                    <select class="form-control selectBasic" name="pay">
                                                        ...
                                                        @foreach ($payment_status as $key => $pay_status)
                                                            <option value="{{ $pay_status }}"
                                                                <?= isset($_GET['pay']) && $pay_status == $_GET['pay'] ? 'selected' : '' ?>>
                                                                {{ $pay_status == 1 ? 'Paid' : ($pay_status == 2 ? 'Pending' : ($pay_status == 3 ? 'Failed' : ($pay_status == 4 ? 'Rejected' : ($pay_status == 5 ? 'Overdue' : 'NA')))) }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-xl-3 col-md-6 mb-3">
                                                    ...
                                                    <select class="form-control selectBasic" name="status">
                                                        ...
                                                        @foreach ($vehicle_status as $key => $vh_status)
                                                            <option value="{{ $vh_status }}"
                                                                <?= isset($_GET['status']) && $vh_status == $_GET['status'] ? 'selected' : '' ?>>
                                                                {{ $vh_status == 1 ? 'Mobilized' : ($vh_status == 2 ? 'Immobilized' : '') }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                            ...
                            <ul>
                                ...
                                <li>
                                    <button type="button" class="btn btn-success waves-effect waves-light"
                                        onclick="exportData('<?= config('table.REF_TABLE.VEHICLES') ?>');">
                                        ...
                                        <tbody>
                                            @foreach ($vehicles as $key => $vehicle)
                                                <tr>
                                                    ...
                                                    <td><a href="{{ route('customer-view', $vehicle->slug) }}"
                                                            title="View Customer"
                                                            style="cursor: pointer;margin-right: 5px;">CUS{{ $vehicle->customer_id }}</a>
                                                    </td>
                                                    <td>{{ $vehicle->phone }}</td>
                                                    <td>{{ $vehicle->hubid }}</td>
                                                    <td>
