            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom bg-white">
                        <h4>Update Product <span class="d-flex heading_label"> {{ ucfirst(request()->route('param')) }}
                            </span> </h4>
                        <div class="go-btn">
                            <a href="{{ route('products', request()->route('param')) }}" class="active" title="Products">Go
                                Back</a>
                        </div>
                    </div>
                            ...
                            <form method="post" id="createProductForm" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input class="form-control" type="text" name="title" id="title"
                                            value="{{ $product->title }}">
                                        <input class="form-control" type="hidden" name="profile_category"
                                            id="profile_category"
                                            value="{{ request()->route('param') == 'corporate' ? 1 : (request()->route('param') == 'individual' ? 2 : (request()->route('param') == 'student' ? 3 : 4)) }}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input class="form-control" type="text" name="ev_number" id="ev_number"
                                            value="{{ $product->ev_number }}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="example-title-input" class="form-label">Chassis Number* &nbsp; <span
                                                class="spanColor chassis_number_error"></label>
                                        <input class="form-control" type="text" name="chassis_number" id="chassis_number"
                                            value="{{ $product->chassis_number }}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="example-title-input" class="form-label">GPS IMEI</label>
                                        <input class="form-control" type="text" name="gps_emei_number"
                                            id="gps_emei_number" value="{{ $product->gps_emei_number }}">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control selectBasic" name="ev_category" id="ev_category">
                                            @foreach ($ev_categories as $key => $ev_category)
                                                <option value="{{ $ev_category }}"
                                                    @if ($ev_category == $product->ev_category_id) selected @endif>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-control selectBasic" name="ev_type_id" id="ev_type_id">
                                            @foreach ($ev_types as $key => $ev_type)
                                                <option value="{{ $ev_type->ev_type_id }}"
                                                    data-value1="{{ $ev_type->speed }}"
                                                    data-value2="{{ $ev_type->range }}"
                                                    data-value3="{{ $ev_type->rs_perday }}"
                                                    data-value4="{{ $ev_type->total_range }}"
                                                    @if ($ev_type->ev_type_id == $product->ev_type_id) selected @endif>
                                                    {{ $ev_type->ev_type_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input class="form-control onlyDigitSpeed" type="text" name="speed"
                                            id="speed" value="{{ $product->speed }}">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label for="range" class="form-label ">Single charge Run Time* (km) &nbsp;<span
                                                class="spanColor onlyDigit_error range_error"
                                                id="range_error"></span></label>
                                        <input class="form-control onlyDigit" type="text" name="km_per_charge"
                                            id="km_per_charge" value="{{ $product->km_per_charge }}">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input class="form-control onlyDigitRent" type="text" name="per_day_rent"
                                            id="per_day_rent" value="{{ $product->per_day_rent }}">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input class="form-control onlyDigitMonthly" type="text" name="total_range"
                                            id="total_range" value="{{ $product->total_range }}">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select class="form-control selectBasic" name="rent_cycle" id="rent_cycle">
                                            @foreach ($rent_cycles as $key => $rent_cycle)
                                                <option value="{{ $rent_cycle }}"
                                                    @if ($rent_cycle == $product->rent_cycle) selected @endif>
                                        ...
                                        <select class="form-control selectBasic" name="bike_type" id="bike_type">
                                            @foreach ($bike_types as $key => $bike_type)
                                                <option value="{{ $bike_type }}"
                                                    @if ($bike_type == $product->bike_type) selected @endif>
                                        ...
                                        <select class="form-control selectBasic" name="battery_type" id="battery_type">
                                            @foreach ($battery_types as $key => $battery_type)
                                                <option value="{{ $battery_type }}"
                                                    @if ($battery_type == $product->bettery_type) selected @endif>
                                        ...
                                        <select class="form-control selectBasic" name="hub_id" id="hub_id">
                                            @foreach ($hubs as $key => $hub)
                                                <option value="{{ $hub->hub_id }}"
                                                    @if ($hub->hub_id == $product->hub_id) selected @endif>{{ $hub->city }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select class="form-control selectBasic" name="status_id" id="status_id">
                                            @foreach ($evStatus as $key => $status_id)
                                                <option value="{{ $status_id }}"
                                                    @if ($status_id == $product->status_id) selected @endif>
                                                    {{ $status_id == 1 ? 'Active' : ($status_id == 2 ? 'Inactive' : ($status_id == 3 ? 'Non Functional' : ($status_id == 4 ? 'Assigned' : ($status_id == 6 ? 'RFD' : '')))) }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <div class="">
                                            <label for="productFile" class="selectImageRemove">
                                                <img class="upload_des_preview clickable selectedImage"
                                                    src="{{ $product->image != '' ? asset('public/upload/product/' . $product->image) : asset('public/assets/images/uploadimg.png') }}"
                                    ...
                                    <div class=" col-md-6 mb-4">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="remember-check"
                                                name="is_display_on_app" @if ($product->is_display_on_app == 1) checked @endif>
                                ...
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <button type="button" class="btn btn-success " id="submitForm">Update</button>
        ...
        $(document).ready(function() {
            ...
            $('#submitForm').click(function(e) {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    url: "{{ route('update-product', $product->slug) }}",
                    data: formDatas,
                    contentType: false,
                    processData: false,
