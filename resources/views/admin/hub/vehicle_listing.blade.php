                    ...
                    <tr>
                        ...
                        <td>
                            @if ($vehicle->customer_id)
                                <a class="customerOverviewModelForm" data-toggle="modal"
                                    data-ev_number="{{ $vehicle->ev_number }}"
                                    data-order_slug="{{ $vehicle->order_slug }}" data-hubid="{{ $hub->hubId }}"
                                    data-chassis_number="{{ $vehicle->chassis_number }}"
                                    data-customer_id="CUS{{ $vehicle->customer_id }}"
                                    data-profile_category_name="{{ $vehicle->profile_category_name }}"
                                    data-ev_category_name="{{ $vehicle->ev_category_name }}"
                                    data-kyc_status="{{ $vehicle->kycStatus }}"
                                    data-cluster_manager="{{ $vehicle->cluster_manager }}"
                                    data-tl_name="{{ $vehicle->tl_name }}"
                                    data-client_name="{{ $vehicle->client_name }}"
                                    data-client_address="{{ $vehicle->client_address }}" title="Customer Overview"
                                    style="cursor: pointer;margin-right: 5px;"> {{ 'CUS' . $vehicle->customer_id }}
                                </a>
                            @else
                                NA
                            @endif
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    @can('edit_inventry', $permission)
                                        <a class="dropdown-item vehicleModelForm" data-toggle="modal"
                                            data-product_id="{{ $vehicle->product_id }}" data-slug="{{ $vehicle->slug }}"
                                            data-hub_id="{{ $vehicle->hub_id }}" data-title="{{ $vehicle->title }}"
                                            data-ev_number="{{ $vehicle->ev_number }}"
                                            data-ev_type_id="{{ $vehicle->ev_type_id }}"
                                            data-ev_category_id="{{ $vehicle->ev_category_id }}"
                                            data-profile_category="{{ $vehicle->profile_category }}"
                                            data-speed="{{ $vehicle->speed }}"
                                            data-rent_cycle="{{ $vehicle->rent_cycle }}"
                                            data-per_day_rent="{{ $vehicle->per_day_rent }}"
                                            data-bettery_type="{{ $vehicle->bettery_type }}"
                                            data-km_per_charge="{{ $vehicle->km_per_charge }}"
                                            data-total_range="{{ $vehicle->total_range }}"
                                            data-description="{{ $vehicle->description }}"
                                            data-is_display_on_app="{{ $vehicle->is_display_on_app }}"
                                            data-chassis_number="{{ $vehicle->chassis_number }}"
                                            data-gps_emei_number="{{ $vehicle->gps_emei_number }}"
                                            data-image="{{ $vehicle->image }}" data-bike_type="{{ $vehicle->bike_type }}"
                                            data-status="{{ $vehicle->status_id }}"
                                            data-updateurl="{{ route('update-product', ['slug' => $vehicle->slug]) }}"
                                            title="Edit Vehicle" style="cursor: pointer;margin-right: 5px;"><i
                                                class="fa fa-edit"></i> Edit
                                        </a>
                                    @endcan
                                    @can('delete_inventry', $permission)
                                        <form id="delete-form-{{ $vehicle->slug }}" method="post"
                                            action="{{ route('product-delete', $vehicle->slug) }}" style="display: none;">
                                            @csrf
...
<div class="modal fade" id="vehicleModelForm" role="dialog" aria-labelledby="modalLabel" data-keyboard="false"
                ...
                <form method="post" enctype="multipart/form-data" id="vehiclForm">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="example-title-input" class="form-label">Title* &nbsp; <span
                                    class="spanColor title_error"></span></label>
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="example-title-input" class="form-label">EV Number* &nbsp; <span
                                    class="spanColor ev_number_error"></label>
                            <input class="form-control" type="text" name="ev_number" id="ev_number"
                                value="">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="example-title-input" class="form-label">Chassis Number* &nbsp; <span
                                    class="spanColor chassis_number_error"></label>
                            <input class="form-control readOnlyClass" type="text" name="chassis_number"
                                id="chassis_number" value="" readonly>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="example-title-input" class="form-label">GPS IMEI</label>
                            <input class="form-control" type="text" name="gps_emei_number" id="gps_emei_number"
                                value="">
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="ev_category" id="ev_category">
                                @foreach ($ev_categories as $key => $ev_category)
                                    <option value="{{ $ev_category }}">
                                        {{ $ev_category == 1 ? 'Two Wheeler' : 'Three Wheeler' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="ev_type_id" class="form-label">EV Type* &nbsp;<span
                                    class="spanColor ev_type_id_error"></span></label>
                            <select class="form-control selectBasic" name="ev_type_id" id="ev_type_id">
                                ...
                                @foreach ($ev_types as $key => $ev_type)
                                    <option value="{{ $ev_type->ev_type_id }}" data-value1="{{ $ev_type->speed }}"
                                        data-value2="{{ $ev_type->range }}" data-value3="{{ $ev_type->rs_perday }}"
                                        data-value4="{{ $ev_type->total_range }}">{{ $ev_type->ev_type_name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="speed" class="form-label">Speed* (km/h) &nbsp;<span
                                    class="spanColor onlyDigitSpeed_error speed_error"
                                    id="speed_error"></span></label>
                            <input class="form-control onlyDigitSpeed" type="text" name="speed" id="speed"
                                value="">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="range" class="form-label ">Single charge Run* (km) &nbsp;<span
                                    class="spanColor onlyDigit_error range_error" id="range_error"></span></label>
                            <input class="form-control onlyDigit" type="text" name="km_per_charge"
                                id="km_per_charge" value="">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="example-title-input" class="form-label">Rent per Day(Rs) &nbsp; <span
                                    class="spanColor onlyDigitRent_error rent_error" id="rent_error"> </span></label>
                            <input class="form-control onlyDigitRent" type="text" name="per_day_rent"
                                id="per_day_rent" value="">
                        </div>
                        <div class="col-md-3 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="profile_category" id="profile_category">
                                @foreach ($profile_categories as $key => $profile_category)
                                    <option value="{{ $profile_category }}">
                                        {{ $profile_category == 1 ? 'Corporate' : ($profile_category == 2 ? 'Individual' : ($profile_category == 3 ? 'Student' : 'Vendor')) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="rent_cycle" id="rent_cycle">
                                @foreach ($rent_cycles as $key => $rent_cycle)
                                    <option value="{{ $rent_cycle }}">
                                        {{ $rent_cycle == 15 ? '15 Days' : '30 Days' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="bike_type" id="bike_type">
                                @foreach ($bike_types as $key => $bike_type)
                                    <option value="{{ $bike_type }}">
                                        {{ $bike_type == 1 ? 'Cargo Bike' : 'Normal Bike' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="battery_type" id="battery_type">
                                @foreach ($battery_types as $key => $battery_type)
                                    <option value="{{ $battery_type }}">
                                        {{ $battery_type == 1 ? 'Swappable' : 'Fixed' }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-8 mb-3">
                            ...
                            <textarea id="description" name="description" class="form-control" rows="6" placeholder="Write here."
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <div class="">
                                ...
                                <input type="file" class="form-control d-none customFile" name="image"
                                    id="customHubFile" />
                            </div>
                        </div>
                        <div class="col-md-6 mb-3">
...
<div class="modal fade" id="customerOverviewModelForm" role="dialog" aria-labelledby="modalLabel"
                ...
                <form method="post" enctype="multipart/form-data" id="customerOverviewForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control readOnlyClass" type="text" id="orderChassisNumber"
                                readonly>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control readOnlyClass" type="text" id="orderClusterManager"
                                readonly>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control readOnlyClass" type="text" id="orderClientAddress"
                                readonly>
                        </div>
                    </div>
                </form>
...
<div class="modal fade" id="vehicleFilterForm" role="dialog" aria-labelledby="modalLabel" data-keyboard="false"
    data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control" type="text" name="ev"
                                value="<?= isset($_GET['ev']) ? $_GET['ev'] : '' ?>">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="example-title-input" class="form-label">Chassis Number</label>
                            <input class="form-control" type="text" name="ch_no"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="ev_cat">
                                ...
                                @foreach ($ev_categories as $key => $ev_category)
                                    <option value="{{ $ev_category }}"
                                        <?= isset($_GET['ev_cat']) && $ev_category == $_GET['ev_cat'] ? 'selected' : '' ?>>
                                        {{ $ev_category == 1 ? 'Two Wheeler' : 'Three Wheeler' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="pro_cat">
                                ...
                                @foreach ($profile_categories as $key => $profile_category)
                                    <option value="{{ $profile_category }}"
                                        <?= isset($_GET['pro_cat']) && $profile_category == $_GET['pro_cat'] ? 'selected' : '' ?>>
                                        {{ $profile_category == 1 ? 'Corporate' : ($profile_category == 2 ? 'Individual' : ($profile_category == 3 ? 'Student' : ($profile_category == 4 ? 'Vendor' : ''))) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="gps">
                                ...
                                @foreach ($gpsDevice as $key => $device)
                                    <option value="{{ $key }}"
                                        <?= isset($_GET['gps']) && $key == $_GET['gps'] ? 'selected' : '' ?>>
                                        {{ $device }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="pay_status">
                                ...
                                @foreach ($paymentStatus as $key => $pay_status)
                                    <option value="{{ $key }}"
                                        <?= isset($_GET['pay_status']) && $key == $_GET['pay_status'] ? 'selected' : '' ?>>
                                        {{ $pay_status }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control selectBasic" name="kyc">
                                ...
                                @foreach ($kycStatus as $key => $kyc_status)
                                    <option value="{{ $key }}"
                                        <?= isset($_GET['kyc']) && $key == $_GET['kyc'] ? 'selected' : '' ?>>
                                        {{ $kyc_status }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <input type="hidden" name="hub_slug" value="{{ $hub->slug }}">
