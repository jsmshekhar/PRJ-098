<?php
$hub_part_request = DB::table('hub_part_accessories')->leftJoin('hubs', 'hubs.hub_id', '=', 'hub_part_accessories.hub_id')->select(
...
$hub_part_request = $hub_part_request->orderBy('hub_part_accessories.created_at', 'DESC')->take(10)->get();
...
@endcan
<div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0 admin_profilt_pop"
            ...
            @foreach ($hub_part_request as $data)
                <div class="d-flex">
                    <div class="flex-grow-1 p-3 border-bottom">
                        @if (Auth::user()->role_id == 0)
                            <p class="notify_text">Hello , you get a Request for Accessories from Hub Id:
                                <b>{{ $data->hubid }} ({{ $data->city }}).</b></p>
                            <div class="notify_btn_box">
                                <form id="delete-form-{{ $data->slug }}" method="post"
                                    action="{{ route('reject-hub-part-accessories', $data->slug) }}"
                                    style="display: none;">
                                    @csrf
                        ...
                        "
                                ...
                                <a href="{{ route('hub-part-accessories') }}"
                                    class="btn btn-success waves-effect waves-light">Open</a>

                            </div>
                        @else
                            <p class="notify_text">
                                @if ($data->status_id == 3)
                                    Hello , your Request for Accessories:
                                    <b>{{ $data->accessories_title }}</b> with a QTY of <b> ({{ $data->assigned_qty }})
                                    </b> has been Assigned.
                                @elseif($data->status_id == 4)
                                    Hello , your Request for Accessories: <b>{{ $data->accessories_title }}</b> with a
                                    QTY of <b> ({{ $data->requested_qty }}) </b>has been Rejected.
                                @endif
                            </p>
                            <div class="notify_btn_box">
                                <a href="{{ route('hub-view', ['slug' => $data->hubSlug, 'param' => 'accessories']) }}"
